...
func realScheduleMonitor(c *Cluster, mon *monConfig) (SchedulingResult, error) {
	...
	d.Spec.Template.Spec.Containers[0].Args = []string{"--", "sleep", "300"}
	...
	for i := 0; i < canaryRetries; i++ {
		...
		if pod.Spec.NodeName == "" {
			logger.Infof("sched-mon: monitor %s canary pod %s not yet scheduled", d.Name, pod.Name)
			time.Sleep(time.Second * canaryRetryDelaySeconds)
			continue
		}
...
func (c *Cluster) assignMons(mons []*monConfig) error {
	...
	for _, mon := range mons {
		...
		if c.HostNetwork || c.spec.Mon.VolumeClaimTemplate == nil {
