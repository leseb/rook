...
func (r *ReconcileCephCluster) watchControllerPredicate() predicate.Funcs {
	return predicate.Funcs{
		CreateFunc: func(e event.CreateEvent) bool {
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			case *cephv1.CephCluster:
				...
				if diff != "" {
					...
					// CHANGE ME
					// Let's try to reconcile and re-initiate the reconcile
					r.cancel()
					...
					// Cancelling the context of the on-going reconciler
					r.cancel()
